...
#define PERIOD_MS (86)//(80) 48 works for f_01
...
void* RoI(void* param){
	...
	while(!abortTest){
		...
		if(gate_RoI){
			...
			cout << "ECEN 5623" << endl;
			cout << "Dr Timothy L Scherr" << endl;
			cout << "Final project by:" << endl;
			cout << "Bryan Cisneros" << endl;
			cout << "Michael Fruge" << endl;
			cout << "Nelson Rodriguez" << endl;
			cout << endl;
...
/*----------------------------acquire--------------------------------*/
  ...
  //rc=pthread_attr_setaffinity_np(&rt_sched_attr_acq, sizeof(cpu_set_t), &threadcpu);
...
/*------------------------------transform------------------------------*/
  ...
  //rc=pthread_attr_setaffinity_np(&rt_sched_attr_xform, sizeof(cpu_set_t), &threadcpu);
...
/*----------------------------calibrate--------------------------------*/
  ...
  //rc=pthread_attr_setaffinity_np(&rt_sched_attr_cal, sizeof(cpu_set_t), &threadcpu);
...
/*--------------------------------RoI-------------------------------*/
  ...
  //rc=pthread_attr_setaffinity_np(&rt_sched_attr_RoI, sizeof(cpu_set_t), &threadcpu);
...
/*--------------------------------proc------------------------------*/
  ...
  //rc=pthread_attr_setaffinity_np(&rt_sched_attr_proc, sizeof(cpu_set_t), &threadcpu);
...
/*---------------------------------logging------------------------------*/
  ...
  //rc=pthread_attr_setaffinity_np(&rt_sched_attr_logging, sizeof(cpu_set_t), &threadcpu);
  ...
  while(1)
  {
	...
	gettimeofday( &initial_time_val, ( struct timezone * )0 );//<----------------------<<time marker start
...
/*--------------------------single time events for calibration------------------------------*/
	if( !gate_OffImage ){
	cout << "Please turn Tiva ON, then press ENTER" << endl;	
	cin.get();
	gate_OnImage= false;
	gate_OffImage= true;
	}

	gettimeofday( &final_time_val, (struct timezone * )0 );//<----------------------<<time marker done
	printf( "Sequencer completed in: %ldus\n", (long)(final_time_val.tv_usec- initial_time_val.tv_usec ));
